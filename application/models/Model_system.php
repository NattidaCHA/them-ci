...
class Model_system extends MY_Model
{
	...
	public function findCustomerByIdVW($cus_no)
	{
		$result = (object)[];
		$sql =  "SELECT TOP 1 * FROM " . VW_Customer . " where mcustno = '$cus_no'";
		$stmt = sqlsrv_query($this->conn, $sql);
	...
	public function defaultCustomerVW()
	{
		$result = [];
		$sql =  "SELECT TOP 5 mcustno,mcustname FROM " . VW_Customer . " group by mcustno,mcustname order by mcustno asc";
		$stmt = sqlsrv_query($this->conn, $sql);
	...
	public function searchCustomerVW($keywork)
	{
		...
		$result = [];
		$sql =  "SELECT mcustno,mcustname  FROM " . VW_Customer;
		$order_by = ' order by mcustno asc';
		if (!empty($keywork)) {
			$sql = $sql . " where mcustno like '%$keywork%' OR mcustname like '%$keywork%'";
		}
		$sql = $sql . $order_by;
		$stmt = sqlsrv_query($this->conn, $sql);
	...
	{
		$result = [];
		$sql =  "SELECT * FROM " . PAYMENT . " order by sort";
		$stmt = sqlsrv_query($this->conn, $sql);
