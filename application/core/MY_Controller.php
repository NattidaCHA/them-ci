...
class MY_Controller extends CI_Controller
{
	...
	public function __construct()
	{
		...
		// $this->uri->segment(empty($http) ? 2 : 3) != 'pdf' && $this->uri->segment(empty($http) ? 2 : 3) != 'cronJob'
		if (!in_array($this->uri->segment(empty($http) ? 2 : 3), ['pdf', 'cronJob', 'lineOA'])) {
	...
	protected function genEmail($params, $page, $checkMain = FALSE)
	{
		...
		if (!empty($params)) {
			...
			$data['data'] = (object)['end_date' => date('d/m/Y', strtotime($params['mduedate'])), 'uuid' => $params['uuid'], 'http' => $this->http, 'contact' => str_replace('สอบถามข้อมูลการชําระเงิน ติดต่อ หน่วยงานบริหารสินเชื่อ ติดต่อ ', '', $tem['page_footer'][0]->contact)];
			...
			if (!empty($cusType->is_email)) {
				if (!empty($emails[$params['cus_no']]) || !empty($emails[$params['cus_main']])) {
					...
					try {
						...
						$title_des =  ((ENVIRONMENT == 'production') ? 'เอกสารใบแจ้งเตือนครบกำหนดชำระค่าสินค้า' : '(ทดสอบระบบ) เอกสารใบแจ้งเตือนครบกำหนดชำระค่าสินค้า');
						...
						if ($cusType->type == 'main' && $page != 'report') {
