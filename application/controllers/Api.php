...
class Api extends MY_Controller
{
	...
	public function addMainLog($action = 'read')
	{
		...
		$result = $this->addSystemLog([
			...
			date('Y-m-d H:i'), $updated_by, $url, $source
		]);
	...
	public function searchCustomerVW()
	{
		...
		if (strlen($keyword) <= 2) {
			if ($apiCustomer = $this->model_system->defaultCustomerVW()->items) {
				if (!empty($apiCustomer)) {
					foreach ($apiCustomer as $val) {
						$result[] = [
							'id' => $val->mcustno,
							'cus_no' => $val->mcustno,
							'cus_name' => $val->mcustname
						];
					}
					$output = ['status' => 200, 'msg' => 'OK', 'results' => $result];
				} else if (empty($apiCustomer)) {
			...
			if ($apiCustomer = $this->model_system->searchCustomerVW($keyword)->items) {
				if (!empty($apiCustomer)) {
					foreach ($apiCustomer as $val) {
						$result[] = [
							'id' => $val->mcustno,
