			...
			table.map(o => {
				...
				if (o.sort == 3) {
					columns.push({
						data: 'send_date',
						render: function(data, type, full) {
							let send_day = full.info.send_date ? days[full.info.send_date.charAt(0).toUpperCase() + full.info.send_date.slice(1)] ? days[full.info.send_date.charAt(0).toUpperCase() + full.info.send_date.slice(1)].name : '-' : '-'
							return send_day;
						}
					})
				}
