		...
		if (!empty($params)) {
			if ($action == 'create') {
				...
				if (empty($checkCustomer)) {
					$customer = [
						genRandomString(16), $params['cus_no'], $params['cus_name'], $params['send_date'], date("Y-m-d H:i:s"), date("Y-m-d H:i:s"), $params['type'], $this->CURUSER->user[0]->userdisplay_th, NULL, !empty($params["is_email"]) ? 1 : 0, !empty($params["is_fax"]) ? 1 : 0, $findCus->msaleorg
					];
